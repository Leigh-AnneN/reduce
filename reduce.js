/*
Write a function called extractValue which accepts an array of objects and a key and returns a new array with the value of each object at 
the key.
...
function vowelCount(str) {
    const vowels = "aeiou";
    return str.split('').reduce(function(acc, nextVal) { //make an array
        let lowerCased = nextVal.toLowerCase();
        if (vowels.indexOf(lowerCased) !== -1) { //if there is a vowel
            if(acc[lowerCased]){
                acc[lowerCased]++;
            }else {
                acc[lowerCased] = 1;
...
/*
Write a function called addKeyAndValue which accepts an array of objects and returns the array of objects passed to it with each object now 
